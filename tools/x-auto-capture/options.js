...
function renderStatus(lastRun, lastArticle) {
  ...
  if (!lastRun) { el.innerHTML = "No sweep run yet. Save your settings, then use “Run now” to test." + art; return; }
  ...
  const perList = Array.isArray(lastRun.results) && lastRun.results.length > 1
    ...
    : "";
  if (lastRun.ok) {
    el.innerHTML = `<span class="ok">✓ Last run ${when}</span>\n${lastRun.lists || 1} target(s) · collected ${lastRun.collected} · imported ${lastRun.imported} new` +
...
async function save() {
  ...
  $("status").innerHTML = `<span class="ok">Saved.</span> ${patch.enabled ? `Runs ${cadence} across ${patch.listUrls.length} target(s).` : "Scheduling disabled."}`;
}

async function runNow() {
  await save();
  $("status").textContent = "Running… (a background tab opens per target, collects, and closes — this can take ~30s each)";
