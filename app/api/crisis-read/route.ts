...
// Crisis map (disasters, hub weather, tropical, NEO), AOR-tagged. Cached 10 min
...
export async function GET() {
  ...
  try {
    ...
    const [threats, advisories] = await Promise.all([getWeatherThreats(locations), getStateAdvisories()]);
    ...
    const prompt = `You are an Air Mobility Command (AMC) watch officer. From this raw signal list, write a tight situation read for an air-mobility audience: what's happening, which AORs/bases it touches, and the airlift implication (HADR pull, NEO/evacuation, weather impeding reach). 3-4 sentences, no preamble, no bullet points, <=90 words. This is coarse open-source SA, not tasking.\n\n${lines.join("\n")}`;
